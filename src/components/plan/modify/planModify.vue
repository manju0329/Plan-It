<script setup>
import planModifyList from './planModifyList.vue';
import planModifyMap from './planModifyMap.vue';
import planModifySearch from './planModifySearch.vue';
import planModifyAttr from './planModifyAttr.vue';
...
import {modifyPlan, nameCheck} from "@/api/plan";
...
const modifyplan = () => {
  if(planName.value == ""){
    ...
    .then(data => {
      ...
      console.log("성공", data);
        if(data > 0){
            if(pstore.planInfo[0].planName != planName.value){
              msg.fire({
              icon : 'error',
              title : '중복된 여행 제목입니다!',
            })
          ...
          for(let i = 0; i < planList.value.length; i++){
            ...
            planList.value[i].planId = pstore.planInfo[0].planId;
...
onMounted(() => {
  if(pstore.plans != null){
    ...
    startDate.value = getFormattedDate(new Date(pstore.planInfo[0].startDate));
    endDate.value = getFormattedDate(new Date(pstore.planInfo[0].endDate));
    planName.value = pstore.planInfo[0].planName;
    console.log(planName.value + " / " + startDate.value);
    pstore.removeAttractions();
  }
})
</script>
        ...
        <div>
              ...
              <div class="form-group">
                <div class="input-group">
                  ...
                  <input type="date" :min="minDate" v-model="startDate" @input="startSelect" data-placeholder="시작날짜" class="form-control" required /> &nbsp;&nbsp;~ &nbsp;&nbsp;
                  <input type="date" :min="minDateEnd" v-model="endDate" @input="endSelect" data-placeholder="종료날짜" class="form-control" required>
                  <input type="text" class="form-control" v-model="planName" id="planName" placeholder="   계획 이름">
                  <button type="submit" class="btn btn-warning" @click.self.prevent="modifyplan">수정하기</button>
                </div>
              </div>
              <div class="path">
                  전체 소요시간 : {{ path.length == 0 ? 0 : (planList.length < 2 ? 0 : Math.round(path[0].duration / 60))}} 분 <br>
